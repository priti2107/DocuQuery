    ...
    <div className="space-y-7">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Review and manage internship and job postings across the platform.
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto inline-flex rounded-full border border-border p-0.5">
          {tabs.map((t, i) => (
            <button
              key={t}
              className={`rounded-full px-3.5 py-1.5 text-xs font-medium transition ${i === 0 ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
            >
              {t}
            </button>
          ))}
      ...
      <div className="surface-card overflow-hidden">
              ...
              >
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center gap-3">
                    <div className="grid h-9 w-9 place-items-center rounded-lg bg-secondary font-serif text-sm font-semibold text-primary">
                      {o.company[0]}
                    </div>
                    <div>
                ...
                <td className="px-5 py-4">
                  <span
                    className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${statusTint[o.status]}`}
                  >
                    {o.status}
                  </span>
                </td>
        ...
        <div className="flex items-center justify-between border-t border-border px-5 py-3 text-xs text-muted-foreground">
          ...
          <div className="flex items-center gap-1">
            <button className="grid h-7 w-7 place-items-center rounded-md hover:bg-muted">
              <ChevronLeft className="h-3.5 w-3.5" />
            </button>
            {["1", "2", "3", "...", "16"].map((p, i) => (
              <button
                key={i}
                className={`grid h-7 min-w-7 place-items-center rounded-md px-1.5 ${p === "1" ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}
              >
                {p}
              </button>
            ))}
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <div className="surface-card bg-gradient-to-br from-primary to-primary/80 p-6 text-primary-foreground">
          <div className="text-xs uppercase tracking-wider text-primary-foreground/70 flex items-center gap-1">
          ...
          <p className="mt-2 text-sm text-primary-foreground/80">
            Engagement is up since last quarter. Consider featuring top-tier
            remote roles to capitalize on trend shifts.
          </p>
        </div>
        <SmallCard
          title="Review Queue"
          desc="12 pending opportunities require approval."
        />
        <SmallCard
          title="Drafts"
          desc="4 job drafts saved by the recruitment team."
        />
      </div>
    </div>
  );
