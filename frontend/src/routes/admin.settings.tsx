...
const notifs = [
  {
    ...
    desc: "Instant alerts for server downtime or security breaches.",
    on: true,
  },
  {
    title: "User Registration Updates",
    desc: "Daily digest of new user accounts and profile approvals.",
    on: false,
  },
  {
    title: "Weekly Analytics Reports",
    desc: "Receive automated PDF summaries of platform growth.",
    on: true,
  },
];
    ...
    <div className="mx-auto max-w-4xl space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Configure platform-wide parameters, security keys, and team
          permissions.
        </p>
      </div>
      ...
      <Section icon={Plug} title="Integration">
        <p className="text-sm text-muted-foreground">
          Manage access tokens for 3rd-party software connections and API calls.
        </p>
        <div className="mt-4">
      ...
      >
        <table className="w-full text-sm">
          <thead className="text-xs uppercase tracking-wider text-muted-foreground">
            <tr>
              <th className="py-3 text-left">Administrator</th>
              ...
              >
                <td className="py-4">
                  <div className="flex items-center gap-3">
                    <div
                      className={`grid h-8 w-8 place-items-center rounded-full text-xs font-semibold ${a.tint}`}
                    >
                      {a.name
                        .split(" ")
                        .map((w) => w[0])
                ...
                <td className="py-4">
                  <span
                    className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${a.tint}`}
                  >
                    {a.role}
                  </span>
                </td>
      ...
      <div className="sticky bottom-4 flex items-center justify-between rounded-2xl border border-border bg-card/95 p-4 shadow-lift backdrop-blur">
        <span className="text-xs italic text-muted-foreground">
          Changes are only permanent after saving config.
        </span>
