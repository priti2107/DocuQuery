      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Detailed engagement metrics for the last 30 days.
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {kpis.map((k, i) => (
          ...
          >
            ...
            <div
              className={`mt-1 inline-flex items-center gap-1 text-xs ${k.up ? "text-primary" : "text-destructive"}`}
            >
        ...
        <div className="surface-card p-6 lg:col-span-2">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-3 text-xs text-muted-foreground">
              ...
              <span className="flex items-center gap-1">
                <span className="h-2 w-2 rounded-full bg-clay" /> Previous
              </span>
            </div>
          </div>
            ...
            {Array.from({ length: 7 * 5 }).map((_, i) => {
              ...
              return (
                <motion.div
                  ...
                  style={{ background: `oklch(0.55 0.06 155 / ${intensity})` }}
                />
              );
            })}
          </div>
          <div className="mt-4 text-xs text-muted-foreground">
            12:00 PM - 2:00 PM is the{" "}
            <span className="font-medium text-primary">Highest Engagement</span>{" "}
            window.
          </div>
        </div>
      </div>
            ...
            {opps.map((o) => (
              ...
              >
                ...
                <td className="px-5 py-3 text-right">
                  <span
                    className={`inline-flex items-center gap-1 ${o.up ? "text-primary" : "text-destructive"}`}
                  >
                    {o.up ? (
                      <TrendingUp className="h-3 w-3" />
                    ) : (
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        ...
        <div className="surface-card bg-gradient-to-br from-primary to-primary/85 p-6 text-primary-foreground">
          <div className="flex items-center gap-2 font-serif text-lg font-semibold">
            <Sparkles className="h-4 w-4" /> Automated Optimization
          </div>
          <p className="mt-2 text-sm text-primary-foreground/85">
            DocuQuery AI has identified a 12% drop in engagement for mobile
            users on the "Categories" page. Would you like to view the suggested
            UI improvements?
          </p>
